...
class ASQPDataModule(pl.LightningDataModule):
    ...
    def load_dataset(self):
        train_dataset = ABSADataset(self.tokenizer, data_dir=args.dataset, data_type=args.dataset_name, max_len=args.max_seq_length,use_prompt=args.use_category_prompt, use_newtarget=args.use_new_target)
        dev_dataset = ABSADataset(self.tokenizer, data_dir=args.dataset, data_type='dev', max_len=args.max_seq_length,use_prompt=args.use_category_prompt, use_newtarget=args.use_new_target)
        test_dataset = ABSADataset(self.tokenizer, data_dir=args.dataset, data_type='test', max_len=args.max_seq_length,use_prompt=args.use_category_prompt, use_newtarget=args.use_new_target)
        self.raw_datasets = {
            'train': train_dataset,
            'dev': dev_dataset,
            'test': test_dataset
...
class T5FineTuner(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        f1 = evaluate(self.data_module.val_dataloader(), self, self.use_new_target, caculate_cate=0, dataset=args.dataset)
...
def evaluate(data_loader, model, use_new_target,caculate_cate, dataset):
    """
    ...
    scores, all_labels, all_preds = compute_scores(outputs, targets, use_new_target, caculate_cate, dataset)
...
if args.do_direct_eval: 
    ...
    scores = evaluate(test_loader, model,args.use_new_target, args.caculate_cate, args.dataset)
    ...
    for data_type_choose in  data_type_for_3:
        ...
        test_loader = DataLoader(test_dataset, batch_size=32, num_workers=4)
        scores = evaluate(test_loader, model, args.use_new_target,caculate_cate=0,dataset=args.dataset)
        log_file_path = f"results_log/{args.dataset}.txt"
        local_time = time.asctime(time.localtime(time.time()))
        exp_settings = f"Datset={args.dataset}; Train bs={args.train_batch_size}, num_epochs = {args.num_train_epochs}"
        exp_results = f"F1 = {scores['f1']:.4f}"
